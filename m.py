...
bot = telebot.TeleBot('7660560466:AAG5HqbPBowsQFLtL7tFFygOxxHUJr0NqYw')
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 4:
                response = "You Are On Cooldown ❌. Please Wait 4 Second Before Running The /aadi Command Again."
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            response = "✅ Usage :- /aadi <target> <port> <time>"  # Updated command syntax
    else:
        response = """❌ You Are Not Authorized To Use This Command ❌.
                      🛒 Please Buy From @user_x_dead"""
...
def welcome_start(message):
    ...
    response = f'''👋🏻Welcome to The Bot, Aadi 💖
